...
function dsum(X::ProductObject, Y::ProductObject)
    return ProductObject(parent(X),Tuple([dsum(x,y) for (x,y) ∈ zip(X.factors, Y.factors)]))
end

function dsum_with_morphisms(X::ProductObject, Y::ProductObject)
    sums = [dsum_with_morphisms(x,y) for (x,y) ∈ zip(X.factors,Y.factors)]
...
function dsum(f::ProductMorphism, g::ProductMorphism)
    ProductMorphism(domain(f)⊕domain(g), codomain(f)⊕codomain(g),Tuple([dsum(fi,gi) for (fi,gi) ∈ zip(f.factors, g.factors)]))
end
